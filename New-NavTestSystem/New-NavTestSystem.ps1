...
if (-not (Get-Module PsLibSqlQueries)) {
...
function Main {
    ...
    if ($execute) {
        if ($backupDestinationDatabase) {
            Get-Content -Path "$tempFolder\00-BackupDestinationDatabase.sql" | Invoke-SqlQuery -dataSource $dstSystem.DataSource -databaseName $dstSystem.DatabaseName
        }
        Get-Content -Path "$tempFolder\01-BackupSourceDatabase.sql" | Invoke-SqlQuery -dataSource $srcSystem[0].DataSource -databaseName $srcSystem[0].DatabaseName
        Get-Content -Path "$tempFolder\02-RestoreIntoDestinationDatabase.sql" | Invoke-SqlQuery -dataSource $dstSystem.DataSource -databaseName $dstSystem.DatabaseName
        Get-Content -Path "$tempFolder\03-PostProcessing.sql" | Invoke-SqlQuery -dataSource $dstSystem.DataSource -databaseName $dstSystem.DatabaseName
